...
import com.xqx.www.view.dialog.OrderRoomDialogFrame;

public class OrderRoomPanel extends CustomPanel{
	...
	public OrderRoomPanel(User user) {
	...
	private void buildAllRoomView(List<Room> rooms, boolean isScroll) {
		...
		for(int i = 0; i < rooms.size(); i++) {
			JLabel label = new JLabel(buildRoomLabel(rooms.get(i)), JLabel.CENTER);
	...
	class LabelListener extends MouseAdapter {
		...
		private Room room;
		private OrderRoomPanel panel;
		
		public LabelListener(OrderRoomPanel panel, Room room) {
			this.panel = panel;
			this.room = room;
		}
		...
		public void mouseClicked(MouseEvent e) {
			try {
				new OrderRoomDialogFrame(panel, room, user);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
		}
